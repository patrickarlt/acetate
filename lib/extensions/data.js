...
module.exports = function (acetate, next){
  ...
  function loadData(filepath, page, callback) {
    ...
    if(ext === '.js') {
      parseModuleData(filepath, page, callback);
      return;
    }
  ...
  function parseModuleData(filepath, page, callback){
    var fullpath = path.join(acetate.root, acetate.src, filepath);
    var data = {};
  ...
  async.each(acetate.pages, function(page, loaded){
    if(page.data){
      async.parallel(_.mapValues(page.data , function(filepath) {
        return _.partial(loadData, filepath, page);

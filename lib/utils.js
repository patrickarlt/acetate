...
function prettyifyUrl(src) {
  var base = stripSlashes(
    src.replace(PRETTY_URL_REGEX, "").replace(/\\/g, "/")
  );
...
function ensureTrailingSlash(baseUrl) {
  ...
  if (!ext && parsedUrl.pathname[parsedUrl.pathname.length - 1] !== "/") {
...
function ensureLeadingSlash(baseUrl) {
  if (
    !PROTOCOL_REGEX.test(baseUrl) &&
    baseUrl[0] !== "/" &&
    baseUrl[0] !== "." &&
    baseUrl[0] !== "#"
  ) {

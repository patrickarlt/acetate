...
function prettyifyUrl (src) {
  var base = stripSlashes(src.replace(PRETTY_URL_REGEX, '')).replace(/\\/g, '/');
  return base ? '/' + base + '/' : '/';
}
...
function mergeMetadata (page, metadata) {
  return _.mergeWith(page, metadata, page.__metadata, function (objValue, srcValue) {
    if (!objValue || (_.isArray(srcValue) && srcValue.length > 0)) {
      return srcValue;
    }
  });
}

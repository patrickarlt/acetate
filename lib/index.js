...
Acetate.prototype.loadPage = function(filepath, callback){
  fs.readFile(filepath, _.bind(function(error, buffer){
    ...
    var metadata = (raw[1]) ? this._parseMetadata(relativePath, raw[1]) : {};
    var config = {
      metadataLines: (raw[1]) ? raw[1].split('\n').length - 1 : 0,
      originalMetadata: metadata,
      dirty: true,
      src: relativePath,
      dest: relativePath.replace(markdownExt, '.html')
    };
    callback(error, new Page(template, metadata, config, this));
  },this));
};
...
Acetate.prototype.build = function(callback){
  this.log.time('build');
  this.log.info('build', 'starting build');

...
Acetate.prototype.helper = function(name, fn){
  ...
  var _fn = _.bind(function() {
    try {
      return fn.apply(this, arguments);
    } catch (e) {
      this.log.error('Error occurred in a helper function %s', name);
      if (e.stack) {
        this.log.error(e.stack);
      } else {
        this.log.error('Error has no stack trace, when throwing exceptions in your helpers always use new Error("message")');
      }
      throw e;
    }
  }, this);
...
Acetate.prototype.loadPage = function(filepath, callback){
  fs.readFile(filepath, _.bind(function(error, buffer){
    ...
    var fileMetadata = this._parseMetadata(relativePath, raw.split(metadataRegex, 2)[1]);

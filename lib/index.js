...
function Acetate(options){
  ...
  this.root = path.dirname(options.config).replace(/\\/g, '/');
...
Acetate.prototype.helper = function(name, fn) {
  ...
  function CustomTag() {
    ...
    this.run = function(context, args){
      ...
      context = args.shift().ctx; // get local variables on page
      var options = args.pop() || {}; // get keyword args (object)
      var newArgs = [context].concat(args); // body, context [args], options
      var result;
...
Acetate.prototype.tag = function(name, fn){
  var acetate = this;
  ...
  function CustomTag() {
    ...
    this.run = function(context, args, body){
      args = Array.prototype.slice.call(arguments);
      body = args.pop(); // body is a function, run it to evaluate.
      context = args.shift().ctx; // get local variables on page
      var options = args.pop() || {}; // get keyword args (object)
      var newArgs = [context, body()].concat(args); // body, context [args], options
      var result;
      newArgs.push(options);
...
Acetate.prototype.loadPage = function(filepath, callback){
  fs.readFile(filepath, _.bind(function(error, buffer){
    var relativePath = filepath.replace(this.root + '/', '').replace(this.src + '/', '');
    var raw = buffer.toString();
    var rawMetadata = raw.split(metadataRegex, 2)[1];
    // console.log(filepath, (rawMetadata && rawMetadata.split('\n').length) ? rawMetadata.split('\n').length : 0);

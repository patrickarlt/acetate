...
function Logger (options) {
  ...
  this._timers = {};
  this.prefix = (this.options.colorize) ? chalk.gray('[') + chalk.blue('ACETATE') + chalk.gray(']') : '[ACETATE]';
  events.EventEmitter.call(this);
}

util.inherits(Logger, events.EventEmitter);

Logger.prototype.colorize = function(level, string){
  return (this.options.colorize) ? colors[level](string) : string;
}
...
Logger.prototype._log = function(level, category, message){
  ...
  var args = ([this.prefix + ' ' + this.colorize(level, '%s') + ' ' + message, category]).concat(rest);

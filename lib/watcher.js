...
AcetateWatcher.prototype._loadNewPage = function(filepath){
  ...
  if(path.basename(filepath)[0] === '_'){
    ...
    this.acetate.log.time('build');
    this.acetate.build(_.bind(function(error){
      if(error){
        this.acetate.log.error('build', 'done in %s with errors', this.acetate.log.timeEnd('build'));
      } else {
        this.acetate.log.success('build', 'done in %s', this.acetate.log.timeEnd('build'));
      }
    }, this));
  } else {

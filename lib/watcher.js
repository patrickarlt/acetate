...
AcetateWatcher.prototype._invalidateNunjucksCache = function(filepath){
  var name = filepath.replace(path.join(this.acetate.root, this.acetate.src) + path.sep, '').replace(path.extname(filepath), '');
  this.acetate.log.debug('nunjucks', 'clearing %s from the template cache', name);

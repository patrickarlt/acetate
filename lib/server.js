      ...
      files.serve(request, response, function (error, result) {
        if (error && error.status === 404 && acetate.notFound) {
          notFoundPage = notFoundPage || _.where(acetate.pages, {
            template: path.join(acetate.src, acetate.notFound)
          })[0].dest;
...
AcetateServer.prototype.start = function(host, port, findPort, open) {
  portscanner.findAPortNotInUse(port, port + MAX_PORTS, host, _.bind(function(error, foundPort) {
    ...
    if (port !== foundPort && findPort === false) {
      ...
      this.acetate.log.success('server', 'started at %s:%s', host, foundPort);
    }

    if(open) {
      this.acetate.log.success('server', 'opening %s:%s', host, foundPort);

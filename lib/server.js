...
function AcetateServer(acetate){
  ...
  this.server = http.createServer(function(request, response){
    request.addListener('end', function() {
      acetate.log.info('server', 'request recived for %s', request.url);
      files.serve(request, response, function (err, result) {
        if (err) {
          acetate.log.error('server', "error serving %s - %s", request.url, err.message);
        } else {
...
AcetateServer.prototype.start = function(host, port, findPort, open) {
  portscanner.findAPortNotInUse(port, port + MAX_PORTS, host, _.bind(function(error, foundPort) {
    ...
    if (port !== foundPort && findPort === false) {
      this.acetate.log.error('server', 'port %s is already in use', port);
      process.exit(1);
    } else {
      this.server.listen(foundPort);
      this.acetate.log.info('server', 'started at %s:%s', host, foundPort);
    }

    if(open) {
      this.acetate.log.info('server', 'opening %s:%s', host, foundPort);

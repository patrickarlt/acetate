...
function buildCallback(acetate){
  ...
  return function(error){
    acetate.log.debug('build', 'building pages done in %s', acetate.log.timeEnd('build'));

    if(error){
      acetate.log.error('build', 'done in %s with errors', acetate.log.timeEnd('start'));
    } else {
      acetate.log.success('build', 'done in %s', acetate.log.timeEnd('start'));
    }

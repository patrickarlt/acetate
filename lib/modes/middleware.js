...
module.exports = function (acetate) {
  ...
  const acetateMiddleware = function acetateMiddleware (request, response, next) {
    ...
    acetate.getPages()
      .then((pages) => _.filter(pages, (page) => page.url === normalizedUrl))
      .then((pages) => {
        if (pages.length > 1) {
          acetate.log.warn(`found ${pages.length} pages matching ${normalizedUrl} (${pages.map(page => page.src).join(',')})`);
        }

        if (pages.length === 0) {
          throw new PageNotFoundError(normalizedUrl);
        }

        return pages[0];
      })
      .catch('PageNotFoundError', (error) => {
        acetate.log.debug(`No page matching ${normalizedUrl} found`);
        next();
        throw error;
      })

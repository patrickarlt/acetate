...
function createServer (acetate, options = {}) {
  ...
  const plugin = {
    ...
    hooks: {
      'client:js': require('fs').readFileSync(path.join(__dirname, '../', 'support', 'client.js'), 'utf8')
    }
  };
  ...
  browserSync.init(options, function (error, bs) {
    ...
    function pageEventHandler (verb) {
      return function (page) {
        acetate.renderer.invalidateTemplate(page.src);
    ...
    function templateEventHandler (verb) {
      return function (src) {
        acetate.renderer.invalidateTemplate(src);
  ...
  return {
    ...
    sendFullscreenMessage: function sendFullscreenMessage (title, message, extra) {
      browserSync.sockets.emit('acetate:fullscreen-message', {
        title,
        message,
        extra

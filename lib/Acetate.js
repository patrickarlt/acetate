...
const { mergeMetadata } = require('./utils');
...
class Acetate extends EventEmitter {
  ...
  transformPage (page) {
    const _masterTimer = this.log.time();
    ...
    return new Promise((resolve, reject) => {
      const done = (e, page) => {
        ...
        this.log.info(`Transformed ${page.src} pages in ${this.log.timeEnd(_masterTimer)}`);
  ...
  _loadDataFromFile (source, done) {
    ...
    if (source.value !== null && source.lastRead > fs.statSync(datapath).mtimeMs) {
  ...
  renderPage (page) {
    ...
    return this._prerenderPage(page)
      ...
      .then(result => {
        this.log.info(`Rendered ${page.url} (${page.src}) in ${this.log.timeEnd(_timer)}`);
        return result;
      });
  }
  ...
  load (pattern, {
    ...
    this._loaders.push(this._createLoaderTask(pattern, {ignore, metadata}));
    ...
    return (pages, callback) => {
      ...
      const done = (error, filepaths) => {
        ...
        const pages = _(filepaths)
          ...
          .map((src) => {
            return createPage.fromTemplate(src, path.join(this.sourceDir, src), metadata);
          })
          .value();

        Promise.all(pages)
          .then((pages) => {
            this.log.info(`Loaded ${pages.length} pages in ${this.log.timeEnd(_timer)}`);
            callback(null, pages);
          })
          .catch((error) => {
            this.log.error(error);
    ...
    if (path.basename(src)[0] === '_') {
      ...
      setTimeout(() => {
        let loader = Object.keys(this._loaderMetadata).find((pattern) => {
          ...
          return minimatch(src, pattern) && (ignores.length > 0) ? ignores.every((pattern) => !minimatch(src, pattern)) : true;
        });
        ...
        Object.keys(this._queries).forEach((name) => {
          ...
          if (minimatch(src, q.pattern)) {
            this._queries[q.name].dirty = true;
          }
        });
  ...
  reloadConfig () {
    ...
    this.prerender('**/*', (page, done) => {
      async.mapValues(this._queries, (q, name, callback) => {
        if (!q.dirty && q.result !== null) {
          this.log.debug(`Using cached result for query:${name}`);
          callback(null, q.result);
          return;
        }

        this.log.debug(`Generating result for query:${name}`);
        this.getPages()
          .then((pages) => {
          ...
          .then(result => {
            ...
            this._queries[q.name].dirty = false;
    ...
    this.prerender('**/*', (page, done) => {
      async.mapValues(this._dataSources, (source, name, callback) => {

...
class Acetate extends EventEmitter {
  ...
  transformPage(page) {
    ...
    return new Promise((resolve, reject) => {
      const done = (e, page) => {
        ...
        this.log.info(
          `Transformed ${page.src} pages in ${this.log.timeEnd(_masterTimer)}`
        );
  ...
  _renderPage(page) {
    if (page.__isMarkdown) {
      return this._renderMarkdown(page.template).then(output => {
  ...
  _createLoaderTask(pattern, { ignore = [], metadata = {} } = {}) {

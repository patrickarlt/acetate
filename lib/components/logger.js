...
module.exports = function (acetate, logLevel) {
  ...
  var processException = function (e) {
    if (e.stack) {
      var firstLine = e.stack.split('\n')[1];
      var components = firstLine.match(/\((.*)\)/g)[0].replace('(', '').replace(')', '').split(':');
  ...
  var formatException = function (error) {
    var e = processException(error);
    return util.format('%s - %s line %d, column %d', e.path, e.message, e.lineNo, e.colNo);
  };

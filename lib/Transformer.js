...
class Transformer {
  constructor(
    { sourceDir = path.join(process.cwd(), "src"), logger, log = "info" } = {}
  ) {
    this._transformers = [];
  ...
  query(name, pattern, mapper, reducer, inital) {
    this.transformAll(pages => {
      let result = _(pages)
        .filter(page => minimatch(page.src, pattern))
        .map(mapper)
        .compact()
        .reduce(reducer, _.clone(inital));

      return this._addMetadataToPages(pages, `queries.${name}`, result);
    });
  }

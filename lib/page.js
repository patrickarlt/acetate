...
Page.prototype.clean = function(callback){
  var buildpath = path.join(this._acetate.options.dest, this.dest);
...
Page.prototype._setupBuild = function(){
  var outputDir =  path.join(process.cwd(), this._acetate.options.dest, path.dirname(this.dest));
...
Page.prototype._write = function (){
  var outputPath = path.join(process.cwd(), this._acetate.options.dest, this.dest);
...
Page.prototype._renderNunjucks = function (template, callback) {
  this._acetate.nunjucks.renderString(template, this, function(error, html){
    callback(error, html);
  });
};

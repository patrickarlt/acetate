  ...
  return function PageFactory(template, metadata){
    var page = Object.create({
      _clean: function(callback){
        var buildpath = path.join(acetate.root, acetate.dest, this.dest);
        ...
        if(this.ignore || !this.__dirty){
          ...
          async.waterfall([
            ...
            if(error){
              ...
              acetate.log.success('page', 'built %s', this.src);
            }
    ...
    var handleBuildError = function(errorMessage){
      ...
      fs.readFile(path.join(__dirname, 'templates', 'error-page.html'), function(error, buffer){

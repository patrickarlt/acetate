...
module.exports = class AcetateConfigError extends Error {
  constructor (error, root) {
    let [file, line, column] = error.stack.split('\n')[0].replace(/^.+Error:\s+/, '').split(':');
    let errorLocation = {
      file, line, column
    };
    ...
    super(`${error.message} at ${errorUtils.formatErrorLocation(errorLocation.file, errorLocation.line, errorLocation.column)}`);

...
module.exports = class PageRenderError extends Error {
  constructor (error, page) {
    ...
    if ((/ \[(Line) \d+, Column \d+\]/).test(error.message)) {
      ...
      let file = (originalErrorLocation) ? path.basename(originalErrorLocation[1]) : page.src;
      let line = parseInt(error.message.match(/Line (\d+)/)[1], 10) + (page.__templateErrorOffset || 0);
      let column = parseInt(error.message.match(/Column (\d+)/)[1], 10);
      super(`${originalMessage} while rendering ${errorUtils.formatErrorLocation(file || page.src, line, column)}`);
      this.line = line;
      this.column = column;

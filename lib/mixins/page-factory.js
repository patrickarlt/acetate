...
module.exports = function (acetate) {
  return function PageFactory (template, metadata) {
    ...
    var clean = function (callback) {
      var buildpath = path.join(acetate.dest, page.dest);
    ...
    var write = function (html, callback) {
      var outputPath = path.join(acetate.dest, page.dest);
      var outputDir = path.join(acetate.dest, path.dirname(page.dest));
    ...
    var renderNunjucks = function (template, callback) {
      ...
      try {
        ...
        if ((/ \[(Line) \d+, Column \d+\]/).test(e.message)) {
          ...
          e.message = message + ' - ' + path.join(acetate.options.src, page.src) + ':' + lineno + ':' + colno;
        } else {

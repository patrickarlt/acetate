...
module.exports = function (acetate) {
  ...
  var parseMetadata = function (filepath, raw) {
    ...
    try {
      ...
      acetate.warn('page', '%s has invalid YAML metadata - %s', filepath, e.message || e);
  ...
  var parseRawTemplate = function (relativepath, buffer) {
    ...
    var _metadataLines = (metadata) ? metadata.split('\n').length : 0;
  ...
  function source (matcher) {
    sources.push(path.join(acetate.root, acetate.src, matcher));
  }

  source('**/*.+(md|markdown|html)');
